...
// Random - 随机数
var Random *RandClass

type RandClass struct{}

// Number - 生成指定长度的随机数
func (c *RandClass) Number(length any) (result string) {
...
// String - 生成随机字符串
func (c *RandClass) String(length any, chars ...string) (result string) {
...
func (c *RandClass) Code(length any, mode ...string) (result string) {
	...
	case "letter":
		return c.String(length, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
	case "mix":
		return c.String(length, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
	default:
		return c.Number(length)
	}
}
...
// Int - 生成随机整数
func (c *RandClass) Int(max any, min ...any) (result int) {
	if Is.Empty(min) {
		min = []any{0}
	}
...
// Slice - 返回随机的指定长度的切片
func (c *RandClass) Slice(slice []any, limit any) (result []any) {
...
// MapSlice - 打乱切片顺序
func (c *RandClass) MapSlice(slice []map[string]any) (result []map[string]any) {
