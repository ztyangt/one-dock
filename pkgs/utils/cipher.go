...
// Sum32 - 哈希加密
func (h *HashClass) Sum32(text any) (result string) {
	item := fnv.New32()
	_, err := item.Write([]byte(cast.ToString(text)))
	if err != nil {
		return ""
	}
	return cast.ToString(item.Sum32())
 ...
 */
func (h *HashClass) Token(value any, length int) (result string) {
	...
	for len(result) < length {
		result += h.Token(result, length-len(result))
	}
 ...
 */
func (h *HashClass) Number(length any) (result string) {
...
// Encrypt 加密
func (a *AESRequest) Encrypt(text any) (result *AESResponse) {
	...
	item := cipher.NewCBCEncrypter(block, []byte(a.Iv))
...
// Decrypt 解密
func (a *AESRequest) Decrypt(text any) (result *AESResponse) {
	...
	item := cipher.NewCBCDecrypter(block, []byte(a.Iv))
 ...
 */
func (r *RSAClass) Generate(bits any) (result *RSAResponse) {
...
// Encrypt 加密
func (r *RSAClass) Encrypt(publicKey, text string) (result *RSAResponse) {
...
// Decrypt 解密
func (r *RSAClass) Decrypt(privateKey, text string) (result *RSAResponse) {
...
func (r *RSAClass) PublicPem(key string) (cert string) {
	...
	if err := pem.Encode(&PEM, block); err != nil {
		return ""
	}
...
func (r *RSAClass) PrivatePem(key string) (cert string) {
